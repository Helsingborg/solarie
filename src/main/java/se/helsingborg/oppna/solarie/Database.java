...
public class Database {
  ...
  public void setup() throws Exception {
    ...
    String localProperties = "/database." + InetAddress.getLocalHost().getHostName() + ".properties";
    try {
      properties.load(getClass().getResourceAsStream(localProperties));
    } catch (Exception e) {
      log.info("No properties file named " + localProperties + ", defaulting to database.properties.");
      properties.load(getClass().getResourceAsStream("/database.properties"));
    }

    String driverClass = properties.getProperty("driver");
    isSQLServer = "com.microsoft.sqlserver.jdbc.SQLServerDriver".equals(driverClass);
    try {
      Class.forName(driverClass);
    } catch (Exception e) {
      log.error("Could not create instance of JDBC driver " + driverClass, e);
      throw e;
    }
  ...
  // todo pooling of connections? there ought to be a lib for that out there
  ...
  public Anvandare getOrCreateAnvändare(String signatur, Connection connection) throws Exception {
    if (signatur == null) {
      return null;
    }
    Anvandare användare = Solarie.getInstance().getPrevayler().execute(new GetAnvandareBySignatur(signatur));
    if (användare != null) {
      return användare;
    }

    användare = Solarie.getInstance().getPrevayler().execute(new CreateAnvandare(signatur));
    // todo load from database

    return användare;
  }

  public Enhet getOrCreateEnhet(String kod, Connection connection) throws Exception {
    if (kod == null) {
      return null;
    }
    Enhet enhet = Solarie.getInstance().getPrevayler().execute(new GetEnhetByKod(kod));
    if (enhet != null) {
      return enhet;
    }

    enhet = Solarie.getInstance().getPrevayler().execute(new CreateEnhet(kod));
    // todo load from database
  ...
  public void importNewEntriesSince(long since) throws Exception {
      ...
      try {
        ...
        try {
          while (rs.next()) {
            ...
            Arende ärende = Solarie.getInstance().getPrevayler().execute(new GetArendeByDiarienummer(diarienummer));
            if (ärende == null) {
              log.info("Skapar ärende med diarienummer " + diarienummer.toString());
              ärende = Solarie.getInstance().getPrevayler().execute(new CreateArende(diarienummer));
            }
            ...
            Enhet enhet = getOrCreateEnhet(rs.getString("enhet_kod"), connection);
            ...
            Anvandare handläggare = getOrCreateAnvändare(rs.getString("usrsign_handl"), connection);
            ...
            Anvandare registrator = getOrCreateAnvändare(rs.getString("usrsign_reg"), connection);
            ...
            Anvandare senasteModifierare = getOrCreateAnvändare(rs.getString("usrsign_reg"), connection);
            ...
            Anvandare ägare = getOrCreateAnvändare(rs.getString("agare_usrsign"), connection);
      ...
      try {
        ...
        try {
          while (rs.next()) {
            ...
            Atgard åtgärd = Solarie.getInstance().getPrevayler().execute(new GetAtgardByDiarienummerAndAtgardsnummer(diarienummer, åtgärdsnummer));
            if (åtgärd == null) {
      ...
      try {
        ...
        try {
          ...
          while (rs.next()) {
            ...
            Enhet enhet = Solarie.getInstance().getPrevayler().execute(new GetEnhetByKod(rs.getString("enhet_kod")));
            if (enhet == null) {
              enhet = Solarie.getInstance().getPrevayler().execute(new CreateEnhet(rs.getString("enhet_kod")));
            }
            ...
            Anvandare ansvarig = getOrCreateAnvändare(rs.getString("ansv_usrsign"), connection);
